...
public class ProductoController {
	 ...
	 */
	/*
	 * se instancian los repositorios, en este caso de productoRepo, proveedorRepo y
	 * categoriaRepo, para poder hacer uso de las funciones del repositorio, las
	 * cuales son la iteración entre la información obtenida en el sistema y la base
	 * de datos.
	 */
	...
	/*
	 * Método que mapea el formulario necesario para hacer el registro de un
	 * producto, el cuál recibe como parametro el modelo a cargar en la plantilla
	 * con el que se va a interactuar.
	 */
	...
	public String showSignUpForm(Model model) {
		...
		/*
		 * Se le agrega a la plantilla un modelo de proveedores ya que requerimos de
		 * ellos para poder hacer una inserción.
		 */
		model.addAttribute("proveedor", iProveedorRepo.findAll());
		/*
		 * Se le agrega a la plantilla un modelo de categorias ya que requerimos de
		 * ellos para poder hacer una inserción.
		 */
	...
	public String addProducto(@Validated Producto producto, BindingResult result, Model model,
		 ...
		 * los datos va a retornar al formulario de agregar productos, de lo contrario
	...
	public String showUpdateForm(@PathVariable("codigoProducto") int codigoProducto, Model model) {
		 ...
		 */
		Producto producto = iProductoRepo.findById(codigoProducto)
				.orElseThrow(() -> new IllegalArgumentException("Invalid departamento id:" + codigoProducto));
		/*
	...
	public String updateProducto(@PathVariable("codigoProducto") int codigoProducto, @Validated Producto producto,
			BindingResult result, Model model, @RequestParam("file") MultipartFile file,
			@RequestParam("cambioUrl") boolean cambioUrl) {
		/*
		...
		/*
		 * Si en la vista principal de actualización se cambia la imagen del producto,
		 * se setea al objeto producto la nueva url y se imprime el resultado.
		 */
		if (cambioUrl) {
			try {
				Map uploadResult = cloudc.upload(file.getBytes(), ObjectUtils.asMap("resourcetype", "auto"));
